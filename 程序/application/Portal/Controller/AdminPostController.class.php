...
class AdminPostController extends AdminbaseController {
	...
	public function add(){
		...
		$this->_getTermTree();
		$term=$this->terms_model->where(array("taxonomy"=>"article"))->where(array('term_id'=>$term_id))->find();
	...
	private function _lists($where=array()){
		...
		$where['post_type']=array(array('eq',4)); // 1 产品 2 单页 3 案例 4 新闻 5 服务 6 下载
			...
			$term=$this->terms_model->where(array('taxonomy'=>'article','term_id'=>$term_id))->find();
			$this->assign("term",$term);
		}
	...
	public function copy(){
	    if(IS_POST){
	        if(isset($_GET['ids']) && isset($_POST['term_id'])){
	            ...
	            foreach ($ids as $id){
	                $find_post=$this->posts_model->field('post_source,post_source_en,post_keywords,post_keywords_en,post_content,post_content_en,post_title,post_title_en,post_excerpt,post_excerpt_en,smeta')->where(array('id'=>$id))->find();
	                if($find_post){
	                    $find_post['post_author']=$uid;
						$find_post['post_type']=4;
