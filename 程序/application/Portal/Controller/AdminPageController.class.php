...
class AdminPageController extends AdminbaseController {
	...
	private function _lists($where=array()){
	    ...
	    $where['post_type']=array('eq',2); // 1 产品 2 单页 3 案例 4 新闻 5 服务 6 下载
	...
	public function add_post(){
		if (IS_POST) {
			...
			$page['post_content_en']=htmlspecialchars_decode($page['post_content_en']);
			$result=$this->posts_model->add($page);
			if ($result) {
				$this->success("添加成功！",U("AdminPage/index"));
			} else {
				$this->error("添加失败！");
			}
		}
	...
	public function edit_post(){
		...
		if (IS_POST) {
			...
			if ($result !== false) {
				$this->success("保存成功！",U("AdminPage/index"));
			} else {
				$this->error("保存失败！");
			}
		}
