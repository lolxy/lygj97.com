...
class AdminProductController extends AdminbaseController {
	...
	// 文章添加
	public function add(){
		$terms = $this->terms_model->where(array("taxonomy"=>"product"))->order(array("listorder"=>"asc"))->select();
	...
	public function edit(){
		...
		$terms=$this->terms_model->where(array("taxonomy"=>"product"))->select();
	...
	private function _lists($where=array()){
		...
		$where['post_type']=array(array('eq',1)); // 1 产品 2 单页 3 案例 4 新闻 5 服务 6 下载
			...
			$term=$this->terms_model->where(array('taxonomy'=>'product','term_id'=>$term_id))->find();
			$this->assign("term",$term);
		}
	...
	private function _getTree(){
		...
		$result = $this->terms_model->where(array('taxonomy'=>'product'))->order(array("listorder"=>"asc"))->select();
	...
	public function move(){
		if(IS_POST){
			...
			$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
			$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
			$terms = $this->terms_model->where(array("taxonomy"=>"product"))->order(array("path"=>"ASC"))->select();
			$new_terms=array();
			foreach ($terms as $r) {
				$r['id']=$r['term_id'];
				$r['parentid']=$r['parent'];
