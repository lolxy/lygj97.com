...
class AdminGongyiController extends AdminbaseController {
	...
	// 文章添加
	public function add(){
		$terms = $this->terms_model->where(array("taxonomy"=>"gongyi"))->order(array("listorder"=>"asc"))->select();
	...
	public function add_post(){
		if (IS_POST) {
			...
			$article['post_content_en']=htmlspecialchars_decode($article['post_content_en']);
	...
	public function edit(){
		...
		$terms=$this->terms_model->where(array("taxonomy"=>"gongyi"))->select();
	...
	private function _lists($where=array()){
		...
		$where['post_type']=array(array('eq',3)); // 1 工程 2 单页 3 工艺 4 文章
			...
			$termsid = empty($termids)?$term_id:$term_id.','.implode(',',$termids);
			$where['term_id']=array('in',"$termsid");
			$term=$this->terms_model->where(array('taxonomy'=>'gongyi','term_id'=>$term_id))->find();
			$this->assign("term",$term);
		}
	...
	private function _getTree(){
		...
		$result = $this->terms_model->where(array('taxonomy'=>'gongyi'))->order(array("listorder"=>"asc"))->select();
	...
	public function move(){
		if(IS_POST){
			...
			$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
			$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
			$terms = $this->terms_model->where(array('taxonomy'=>'gongyi'))->order(array("path"=>"ASC"))->select();
			$new_terms=array();
			foreach ($terms as $r) {
				$r['id']=$r['term_id'];
				$r['parentid']=$r['parent'];
	...
	public function copy(){
	    if(IS_POST){
	        if(isset($_GET['ids']) && isset($_POST['term_id'])){
	            ...
	            $term_id=I('post.term_id',0,'intval');
	            $term_count=$terms_model=M('Terms')->where(array('taxonomy'=>'gongyi'))->where(array('term_id'=>$term_id))->count();
	            if($term_count==0){
	                $this->error('分类不存在！');
	            }
	            ...
	            foreach ($ids as $id){
	                $find_post=$this->posts_model->field('post_keywords,post_keywords_en,post_content,post_content_en,post_title,post_title_en,post_excerpt,post_excerpt_en,smeta')->where(array('id'=>$id))->find();
	                if($find_post){
	                    $find_post['post_author']=$uid;
						$find_post['post_type']=3;
