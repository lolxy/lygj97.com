...
class Page {
    ...
    public function Pager($Page_tpl = '') {
        ...
        if ($cfg ['pageindex'] > 1) {
			...
			$this->pFirst = $this->linkwraper_pre.'<a href="javascript:void(0);" class="disabled">' . $cfg ['first'] . '</a> '.$this->linkwraper_after; //首页
			$this->pPrev = $this->linkwraper_pre.'<a href="javascript:void(0);" class="disabled">' . $cfg ['prev'] . '</a> '.$this->linkwraper_after; //上一页
			}
        //下一页，尾页
        if ($cfg ['pageindex'] < $cfg ['pagecount']) {
			...
			$this->pLast = $this->linkwraper_pre.'<a href="javascript:void(0);" class="disabled">' . $cfg ['last'] . '</a> '.$this->linkwraper_after;
            $this->pNext = $this->linkwraper_pre.'<a href="javascript:void(0);" class="disabled">' . $cfg ['next'] . '</a> '.$this->linkwraper_after;
			}
